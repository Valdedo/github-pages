...
def clone_document(doc_id: int, db: Session = Depends(get_db)):
    """Clone a document and all its articles (without re-running OCR)."""
    orig = db.query(Document).filter(Document.id == doc_id).first()
    if not orig:
        raise HTTPException(404, "Document not found")

    orig_articles = (
        db.query(Article)
        .filter(Article.document_id == doc_id)
        .order_by(Article.line_number)
        .all()
    )

    # Copy the physical file
    orig_path = Path(orig.file_path)
    if orig_path.exists():
        new_stored = f"{uuid.uuid4().hex}{orig_path.suffix}"
        new_path = orig_path.parent / new_stored
        shutil.copy2(orig_path, new_path)
    else:
        new_stored = orig.filename
        new_path = orig_path

    new_doc = Document(
        filename=new_stored,
        original_filename=f"[COPIA] {orig.original_filename}",
        ...
        pronto_pago_pct=orig.pronto_pago_pct,
    )
    db.add(new_doc)
    ...
    for art in orig_articles:
        new_art = Article(
            ...
            precio_unitario_bruto=art.precio_unitario_bruto,
            descuento_1=art.descuento_1,
            descuento_2=art.descuento_2,
            descuento_3=art.descuento_3,
            descuento_4=art.descuento_4,
            coste_neto_unitario=art.coste_neto_unitario,
            coste_neto_total=art.coste_neto_total,
            iva_pct=art.iva_pct,
            recargo_pct=art.recargo_pct,
            margen_pct=art.margen_pct,
            margen_override=art.margen_override,
            pvp_sin_iva=art.pvp_sin_iva,
            pvp_con_iva=art.pvp_con_iva,
            codigo_proveedor=art.codigo_proveedor,
            codigo_fabricante=art.codigo_fabricante,
            ean=art.ean,
            codigo_principal=art.codigo_principal,
            otros_codigos=art.otros_codigos,
        )
        db.add(new_art)

    db.commit()
    db.refresh(new_doc)
    return {"id": new_doc.id, "original_filename": new_doc.original_filename}
