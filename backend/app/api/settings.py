...
def update_settings(update: AppSettingsUpdate, db: Session = Depends(get_db)):
    ...
    if update.rounding_mode is not None:
        if update.rounding_mode not in ("standard", "psychological", "ceil_5cents", "ceil_10cents"):
            raise HTTPException(400, "rounding_mode must be 'standard', 'psychological', 'ceil_5cents' or 'ceil_10cents'")
